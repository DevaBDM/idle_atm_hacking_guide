    ...
    while (true) {
      if (need_restart_) {
        ...
        process_response(client_manager_->receive(10));
          ...
          message_content->text_->text_ = std::move("🆓 Free Spin " + stringSpin + "🎰");
...
int main() {
  ...
  {
    runSucc = false;
    example.loop(freeSpin);
    (( runSucc ) ? (--tooMuchError,--freeSpin):++tooMuchError);
    sleep(1);
    if ( tooMuchError > 2 ) 
      break;
    else if ( tooMuchError < 0 )
      tooMuchError=0;
  }
}
