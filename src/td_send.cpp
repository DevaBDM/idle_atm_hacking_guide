          ...
          regex_str_to_match = "🏧 Enter a (.*)-digit number - PIN code";
            while (true) {
                ...
                if (response.object) {
                  ...
                  if (found_what_you_need == 1 && found_result_of_guess == 1){
                      found_what_you_need=0;
                      found_result_of_guess= 0;
                      sent_number_message = 1;
                      regex_str_to_match = "empty";
                      break;
                  } else if (found_what_you_need == 2) {
                    std::cout << "ERROR" << std::endl;
                    sent_number_message = 2;
                    found_what_you_need=0;
                    break;
                  } else if ( found_what_you_need == 3 ) {
    ...
    td_api::downcast_call(
        *update, overloaded(
                     ...
                     [this](td_api::updateNewMessage &update_new_message) {
                       ...
                       if (std::regex_match (text, std::regex("[0X_]+"))){
                        found_result_of_guess=1;
                        std::cout << text << std::endl;
                       } else if (std::regex_match (head_one, regex_str_to_match)){
                        found_what_you_need=1;
                        //std::cout << text << std::endl;
                       } else if (std::regex_match (head_one, std::regex("🤔"))){
                         found_what_you_need=2;
                        //std::cout << text << std::endl;
                       } else if (std::regex_match (head_one, std::regex("⛔️ Attempts ended 😟"))){
                         found_what_you_need=3;
                        //std::cout << text << std::endl;
                       }
                     }
                     },
